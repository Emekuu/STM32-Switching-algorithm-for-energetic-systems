...
static BLEMetricsFrame_t last_ble_metrics;
static uint8_t ble_frame_valid = 0;
static uint32_t ble_frames_received = 0;
/* USER CODE END PV */
...
static SVCCTL_EvtAckStatus_t Event_Handler(void *Event)
{
  ...
  switch(event_pckt->evt)
  {
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
    {
      ...
      switch(blecore_evt->ecode)
      {
            ...
            if (pr->Format == UUID_TYPE_16)
            {
              for (i=0; i<numDesc; i++)
              {
                ...
                if(uuid == CLIENT_CHAR_CONFIG_DESCRIPTOR_UUID)
                {
                	...
                	APP_DBG_MSG("CCCD found handle=0x%x state=%d\r\n",
                	                    handle, aP2PClientContext[index].state);
#if(CFG_DEBUG_APP_TRACE != 0)
        ...
        {
          APP_DBG_MSG("HIT ACI_GATT_NOTIFICATION_VSEVT_CODE\r\n");
  ...
  {APP_DBG_MSG("CLIENT Gatt_Notification called, len=%d first=0x%02X\r\n",
    ...
    switch(pNotification->P2P_Client_Evt_Opcode)
    {
      ...
      {
    	  if (pNotification->DataTransfered.Length == sizeof(BLEMetricsFrame_t))
    	  {
    	    memcpy(&last_ble_metrics,
    	           pNotification->DataTransfered.pPayload,
    	           sizeof(last_ble_metrics));

    	    if (BLEMetricsChecksum((const uint8_t*)&last_ble_metrics,
    	                           sizeof(last_ble_metrics) - 1) ==
    	        last_ble_metrics.checksum)
    	    {
    	      ble_frame_valid = 1;
    	      ble_frames_received++;
    	      ...
    	      APP_DBG_MSG("BLE frame OK: rssi=%d loss=%u rtt=%u jitter=%u\n\r",
    	                  last_ble_metrics.rssi,
    	                  last_ble_metrics.loss_permille,
    	                  last_ble_metrics.rtt_ms,
    	                  last_ble_metrics.jitter_ms);
    	    }
    	    else
    	    {
    	      ble_frame_valid = 0;
    	      BSP_LED_On(LED_RED);
    	      APP_DBG_MSG("BLE frame checksum error\n\r");
    	    }
    	  }
        else
        {
          APP_DBG_MSG("BLE frame unexpected len=%d\n\r",
                      pNotification->DataTransfered.Length);
        }
        break;
      }
