...
static void I2C_SmokeTest(void);
/* USER CODE BEGIN PFP */
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
static void WB_I2C_BusRecovery(void)
{
...
static void WB_BusProbe(void)
{
    GPIO_InitTypeDef g = {0};
    __HAL_RCC_GPIOB_CLK_ENABLE();

    // PB8=SCL, PB9=SDA — správne I2C piny
    g.Pin  = GPIO_PIN_8 | GPIO_PIN_9;
    ...
    printf("[WB_PROBE] PULLDOWN: SCL=%u SDA=%u\r\n", scl_pd, sda_pd);
    printf("[WB_PROBE] PULLUP:   SCL=%u SDA=%u\r\n", scl_pu, sda_pu);
...
{
  uint8_t tx = 0xA5;
  uint16_t addr = (0x12 << 1);  // 0x24, F7 slave

  HAL_StatusTypeDef ret = HAL_I2C_Master_Transmit(&hi2c1, addr, &tx, 12, 1000);

  if (ret != HAL_OK) {
    uint32_t err = HAL_I2C_GetError(&hi2c1);
    printf("[WB smoke] ret=%d err=0x%08lX addr=0x%02X\r\n",
           ret, (unsigned long)err, addr);
  } else {
    printf("[WB smoke] OK addr=0x%02X\r\n", addr);
  }
}
/**
