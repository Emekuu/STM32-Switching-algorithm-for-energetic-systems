...
/* USER CODE BEGIN PM */
#define F767_I2C_ADDR      (0x12 << 1)   // uprav podľa tvojho slave
#define I2C_FWD_MAX_LEN    244
#define I2C_FWD_TIMEOUT_MS 100
/* USER CODE END PM */
...
static uint8_t Client_ParseFrame(const uint8_t *payload,
                                 uint16_t len,
 ...
 *
 *************************************************************/
static void WB_Test_I2C_Ping(void)
{
    uint8_t test[1] = {0xA5};
    HAL_StatusTypeDef ret;
    uint32_t err;

    ret = HAL_I2C_Master_Transmit(&hi2c1, F767_I2C_ADDR, test, sizeof(test), 100);

    if (ret != HAL_OK)
    {
        err = HAL_I2C_GetError(&hi2c1);
        APP_DBG_MSG("[PING] I2C TX error: ret=%d err=0x%08lX addr=0x%02X\n",
                    ret, err, F767_I2C_ADDR);
    }
    else
    {
        APP_DBG_MSG("[PING] I2C TX OK addr=0x%02X\n", F767_I2C_ADDR);
    }
}
...
static void ForwardMetricsToF767(const generic_frame_info_t *info,
                                  const uint8_t *raw, uint16_t len)
{
    uint16_t send_len = (len > I2C_FWD_MAX_LEN) ? I2C_FWD_MAX_LEN : len;
    HAL_StatusTypeDef ret = HAL_I2C_Master_Transmit(
        &hi2c1, F767_I2C_ADDR,
        (uint8_t*)raw, send_len,
        I2C_FWD_TIMEOUT_MS);
    APP_DBG_MSG("[FWD] seq=%lu ret=%d\n", (unsigned long)info->seq, ret);
}
static SVCCTL_EvtAckStatus_t DTC_Event_Handler( void *Event )
{
...
//            pr->Data_Length -= 1;
        ...
        case ACI_GATT_NOTIFICATION_VSEVT_CODE:
        {
          ...
          APP_DBG_MSG("[NOTIF] handle=0x%04X TX=0x%04X TP=0x%04X\n",
                          pr->Attribute_Handle,
                          DTC_Context.TXCharHdle,
                          DTC_Context.ThroughputCharHdle);
          if ((pr->Attribute_Handle == DTC_Context.TXCharHdle) &&
              (pr->Attribute_Value_Length >= sizeof(lab_test_hdr_t)))
          {
            ...
            if (Client_ParseFrame(pr->Attribute_Value,
                                  ...
                                  &info))
            {
                ...
                DataTransfered += pr->Attribute_Value_Length;
                ForwardMetricsToF767(&info, pr->Attribute_Value, pr->Attribute_Value_Length);
            }
            else
            {
              APP_DBG_MSG("Parse error, len=%u\n", pr->Attribute_Value_Length);
            }
          }
        }
        break;
/* end ACI_GATT_NOTIFICATION_VSEVT_CODE */
